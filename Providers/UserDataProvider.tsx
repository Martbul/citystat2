...
import { UserData, UserDataContextType, Role, Status, CityStat, StreetWalk } from "@/types/user";
import { Theme, Language, Settings, TextSize, MessagesAllowance, RoleColors, Motion, StickersAnimation } from "@/types/settings";
        ...
        try {
          ...
          if (error.message === "USER_NOT_FOUND") {
            ...
            const newUserData = {
              ...
              imageUrl: user.imageUrl || "https://48htuluf59.ufs.sh/f/1NvBfFppWcZeWF2WCCi3zDay6IgjQLVNYHEhKiCJ8OeGwTon",
            ...
            console.log("New user created:", fetchedUserData.id);
          } else {
            throw error;
          }
        }
...
const updateAboutMe = useCallback(async (aboutMe: string): Promise<void> => {
  await updateUser({ aboutMe });
}, [updateUser]);

const setDisableAccount = useCallback(async (disabled: boolean): Promise<void> => {
  await updateUser({ disableAccount: disabled });
}, [updateUser]);

const setDeleteAccount = useCallback(async (deleted: boolean): Promise<void> => {
  await updateUser({ deleteAccount: deleted });
}, [updateUser]);

const updateCityStats = useCallback(async (
...
const addStreetWalk = useCallback(async (
...
const addKilometers = useCallback(async (km: number): Promise<void> => {
  if (!userData?.cityStats) return;
...
const updateFirstName = useCallback(async (firstName: string): Promise<void> => {
  await updateUserField('firstName', firstName);
}, [updateUserField]);

const updateLastName = useCallback(async (lastName: string): Promise<void> => {
  await updateUserField('lastName', lastName);
}, [updateUserField]);


const updateNote = useCallback(async (note: string): Promise<void> => {
  await updateUserField('note', note);
}, [updateUserField]);

// Settings-specific updates
const updateTextSize = useCallback(async (textSize: TextSize): Promise<void> => {
  await updateSettings({ textSize });
}, [updateSettings]);

const updateZoomLevel = useCallback(async (zoomLevel: string): Promise<void> => {
  await updateSettings({ zoomLevel });
}, [updateSettings]);

const updateFontStyle = useCallback(async (fontStyle: string): Promise<void> => {
